"""Test nflverse player-master height and weight in weekly-template matching."""

from __future__ import annotations

import argparse
import importlib.util
import json
import sqlite3
import sys
import time
from copy import deepcopy
from pathlib import Path

import numpy as np
import pandas as pd


STUDY_DIR = Path(__file__).resolve().parent
REPO_ROOT = STUDY_DIR.parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

REFERENCE_PATH = (
    REPO_ROOT
    / "research"
    / "studies"
    / "2026-07-30_template_receiver_rate_ablation"
    / "run_validation.py"
)
SPEC = importlib.util.spec_from_file_location(
    "height_weight_receiver_rate_reference",
    REFERENCE_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise ImportError(f"Could not import template replay from {REFERENCE_PATH}")
reference = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = reference
SPEC.loader.exec_module(reference)
pruning = reference.pruning
base = reference.base
builder = reference.builder

from Scripts.V2.config import NFLVERSE_PLAYERS_URL


DEFAULT_RESULTS = STUDY_DIR / "results"
BASELINE_METHOD = "production"
PRIMARY_METHOD = "height_weight_w025_all"
RECENCY_HALF_LIFE = 12.0
BOOTSTRAP_SAMPLES = 2_000
PERIODS = {
    "all_2017_2025": (2017, 2025),
    "development_2017_2022": (2017, 2022),
    "recent_2020_2025": (2020, 2025),
    "temporal_2023_2025": (2023, 2025),
}
BOOTSTRAP_PERIODS = {
    "all_2017_2025": (2017, 2025),
    "development_2017_2022": (2017, 2022),
    "temporal_2023_2025": (2023, 2025),
}
HEIGHT_FEATURE = "match_height_position_pct"
WEIGHT_FEATURE = "match_weight_position_pct"
RAW_MEASURABLE_COLUMNS = [
    "height_inches",
    "weight_lbs",
    "measurable_source_position",
    "measurable_join_method",
    "height_available",
    "weight_available",
    "measurables_available",
    "measurable_position_mismatch",
]
PROFILE_COLUMNS = [
    *RAW_MEASURABLE_COLUMNS,
    HEIGHT_FEATURE,
    WEIGHT_FEATURE,
]
VARIANTS = {
    BASELINE_METHOD: {
        "height_weight": 0.0,
        "weight_weight": 0.0,
        "positions": (),
        "primary": 0,
    },
    "height_w025_all": {
        "height_weight": 0.25,
        "weight_weight": 0.0,
        "positions": tuple(builder.POSITIONS),
        "primary": 0,
    },
    "weight_w025_all": {
        "height_weight": 0.0,
        "weight_weight": 0.25,
        "positions": tuple(builder.POSITIONS),
        "primary": 0,
    },
    PRIMARY_METHOD: {
        "height_weight": 0.25,
        "weight_weight": 0.25,
        "positions": tuple(builder.POSITIONS),
        "primary": 1,
    },
    "height_weight_w050_all": {
        "height_weight": 0.50,
        "weight_weight": 0.50,
        "positions": tuple(builder.POSITIONS),
        "primary": 0,
    },
    "height_weight_w025_skill": {
        "height_weight": 0.25,
        "weight_weight": 0.25,
        "positions": ("RB", "WR", "TE"),
        "primary": 0,
    },
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--league", default="dk")
    parser.add_argument(
        "--v2-db",
        type=Path,
        default=None,
        help=(
            "League-specific V2 database containing player_identity. "
            "Defaults to the configured database for --league."
        ),
    )
    parser.add_argument("--results-dir", type=Path, default=DEFAULT_RESULTS)
    return parser.parse_args()


def build_methods() -> tuple[dict[str, dict], pd.DataFrame]:
    methods: dict[str, dict] = {}
    metadata = []
    for method, variant in VARIANTS.items():
        weights = deepcopy(builder.MATCH_FEATURE_WEIGHTS)
        for position in variant["positions"]:
            if variant["height_weight"] > 0:
                weights[position][HEIGHT_FEATURE] = variant["height_weight"]
            if variant["weight_weight"] > 0:
                weights[position][WEIGHT_FEATURE] = variant["weight_weight"]
        methods[method] = {
            "weights": weights,
            "recency_half_life": RECENCY_HALF_LIFE,
            "variant": method,
            "removed_families": (),
        }
        metadata.append(
            {
                "method": method,
                "height_weight": variant["height_weight"],
                "weight_weight": variant["weight_weight"],
                "positions": ",".join(variant["positions"]),
                "primary": variant["primary"],
                "recency_half_life": RECENCY_HALF_LIFE,
                "total_match_weight": sum(
                    sum(position_weights.values())
                    for position_weights in weights.values()
                ),
                **{
                    f"{position.lower()}_feature_count": len(weights[position])
                    for position in builder.POSITIONS
                },
            }
        )
    return methods, pd.DataFrame(metadata)


METHODS, METHOD_METADATA = build_methods()
BOOTSTRAP_METHODS = tuple(
    method for method in METHODS if method != BASELINE_METHOD
)


def _clean_id(series: pd.Series) -> pd.Series:
    return (
        series.astype("string")
        .str.strip()
        .replace({"": pd.NA, "nan": pd.NA, "None": pd.NA})
    )


def load_measurables(v2_database: Path) -> pd.DataFrame:
    with sqlite3.connect(v2_database) as connection:
        identity = pd.read_sql_query(
            """
            SELECT player_key, gsis_id, pfr_id, position
            FROM player_identity
            """,
            connection,
        )
    if identity["player_key"].duplicated().any():
        raise ValueError("player_identity is not unique by player_key.")

    players = pd.read_csv(
        NFLVERSE_PLAYERS_URL,
        dtype={"gsis_id": "string", "pfr_id": "string"},
        low_memory=False,
    )
    required = {"gsis_id", "pfr_id", "height", "weight"}
    missing = sorted(required.difference(players.columns))
    if missing:
        raise ValueError(
            "nflverse players is missing measurable columns: "
            + ", ".join(missing)
        )
    for frame in (identity, players):
        frame["gsis_id"] = _clean_id(frame["gsis_id"])
        frame["pfr_id"] = _clean_id(frame["pfr_id"])
    if players.loc[players.gsis_id.notna(), "gsis_id"].duplicated().any():
        raise ValueError("nflverse players contains duplicate non-null gsis_id.")
    if players.loc[players.pfr_id.notna(), "pfr_id"].duplicated().any():
        raise ValueError("nflverse players contains duplicate non-null pfr_id.")

    gsis_source = players[players.gsis_id.notna()].set_index("gsis_id")
    pfr_source = players[players.pfr_id.notna()].set_index("pfr_id")
    gsis_height = identity.gsis_id.map(gsis_source["height"])
    gsis_weight = identity.gsis_id.map(gsis_source["weight"])
    pfr_height = identity.pfr_id.map(pfr_source["height"])
    pfr_weight = identity.pfr_id.map(pfr_source["weight"])

    output = identity[["player_key", "position"]].rename(
        columns={"position": "measurable_source_position"}
    )
    output["height_inches"] = pd.to_numeric(
        gsis_height.combine_first(pfr_height),
        errors="coerce",
    )
    output["weight_lbs"] = pd.to_numeric(
        gsis_weight.combine_first(pfr_weight),
        errors="coerce",
    )
    gsis_match = gsis_height.notna() | gsis_weight.notna()
    pfr_match = ~gsis_match & (pfr_height.notna() | pfr_weight.notna())
    output["measurable_join_method"] = np.select(
        [gsis_match, pfr_match],
        ["nflverse_gsis_id", "nflverse_pfr_id"],
        default="unavailable",
    )
    invalid_height = output.height_inches.notna() & ~output.height_inches.between(
        60, 84
    )
    invalid_weight = output.weight_lbs.notna() & ~output.weight_lbs.between(
        130, 420
    )
    if invalid_height.any() or invalid_weight.any():
        raise ValueError("nflverse player-master measurables failed range checks.")
    return output


def attach_measurable_features(
    frame: pd.DataFrame,
    measurables: pd.DataFrame,
) -> pd.DataFrame:
    overlapping = sorted(set(PROFILE_COLUMNS).intersection(frame.columns))
    if overlapping:
        raise ValueError(
            "Measurable output columns already exist: " + ", ".join(overlapping)
        )
    output = frame.merge(
        measurables,
        on="player_key",
        how="left",
        validate="many_to_one",
    )
    output["height_available"] = output.height_inches.notna().astype(int)
    output["weight_available"] = output.weight_lbs.notna().astype(int)
    output["measurables_available"] = (
        output.height_available.eq(1) & output.weight_available.eq(1)
    ).astype(int)
    output["measurable_position_mismatch"] = (
        output.measurable_source_position.notna()
        & output.pos.notna()
        & output.measurable_source_position.ne(output.pos)
    ).astype(int)
    output[HEIGHT_FEATURE] = (
        output.groupby(["season", "pos"], observed=True)["height_inches"]
        .rank(method="average", pct=True)
        .fillna(builder.MATCH_FILL_VALUE)
        .clip(lower=0, upper=1)
    )
    output[WEIGHT_FEATURE] = (
        output.groupby(["season", "pos"], observed=True)["weight_lbs"]
        .rank(method="average", pct=True)
        .fillna(builder.MATCH_FILL_VALUE)
        .clip(lower=0, upper=1)
    )
    output["measurable_join_method"] = output[
        "measurable_join_method"
    ].fillna("unavailable")
    return output


def coverage_audit(
    templates: pd.DataFrame,
    targets: pd.DataFrame,
) -> pd.DataFrame:
    rows = []
    for population, frame in [
        ("historical_templates", templates),
        ("rolling_targets", targets),
    ]:
        for position, group in frame.groupby("pos", sort=True):
            rows.append(
                {
                    "population": population,
                    "pos": position,
                    "rows": len(group),
                    "height_available": int(group.height_available.sum()),
                    "weight_available": int(group.weight_available.sum()),
                    "both_available": int(group.measurables_available.sum()),
                    "both_coverage": float(group.measurables_available.mean()),
                    "position_mismatches": int(
                        group.measurable_position_mismatch.sum()
                    ),
                }
            )
    return pd.DataFrame(rows)


def attach_target_profile(
    predictions: pd.DataFrame,
    targets: pd.DataFrame,
) -> pd.DataFrame:
    profile = targets[
        ["player_key", "player", "pos", "season", *PROFILE_COLUMNS]
    ].rename(columns={"player_key": "target_player_key"})
    return predictions.merge(
        profile,
        on=["player", "pos", "season"],
        how="left",
        validate="many_to_one",
    )


def scope_frame(frame: pd.DataFrame, scope: str) -> pd.DataFrame:
    if scope == "all":
        return frame
    if scope == "wr_te":
        return frame[frame.pos.isin(["WR", "TE"])]
    return frame[frame.pos.eq(scope.upper())]


def grouped_period_summaries(
    predictions: pd.DataFrame,
    scope: str,
) -> pd.DataFrame:
    output = []
    scoped = scope_frame(predictions, scope)
    for period, (start, end) in PERIODS.items():
        frame = scoped[scoped.season.between(start, end)]
        summary = pruning.grouped_summary(frame, ["method"])
        summary.insert(0, "scope", scope)
        summary.insert(1, "period", period)
        output.append(summary)
    return pd.concat(output, ignore_index=True).merge(
        METHOD_METADATA,
        on="method",
        how="left",
        validate="many_to_one",
    )


def clustered_bootstrap(predictions: pd.DataFrame) -> pd.DataFrame:
    key_cols = ["target_player_key", "pos", "season"]
    metric_columns = pruning.LOWER_IS_BETTER
    output = []
    for scope in ["all", "qb", "rb", "wr", "te", "wr_te"]:
        scoped = scope_frame(predictions, scope)
        baseline = scoped[scoped.method.eq(BASELINE_METHOD)][
            key_cols + metric_columns
        ]
        for method in BOOTSTRAP_METHODS:
            candidate = scoped[scoped.method.eq(method)][
                key_cols + metric_columns
            ]
            paired = candidate.merge(
                baseline,
                on=key_cols,
                suffixes=("_candidate", "_baseline"),
                validate="one_to_one",
            )
            for period, (start, end) in BOOTSTRAP_PERIODS.items():
                frame = paired[paired.season.between(start, end)].copy()
                for cluster_type, cluster_column in [
                    ("season", "season"),
                    ("player", "target_player_key"),
                ]:
                    clusters = frame[cluster_column].drop_duplicates().tolist()
                    cluster_index = {
                        cluster: index
                        for index, cluster in enumerate(clusters)
                    }
                    cluster_ids = (
                        frame[cluster_column].map(cluster_index).to_numpy()
                    )
                    rng = np.random.default_rng(
                        builder.stable_seed(
                            "height_weight_bootstrap",
                            scope,
                            method,
                            period,
                            cluster_type,
                        )
                    )
                    sampled_index = rng.integers(
                        0,
                        len(clusters),
                        size=(BOOTSTRAP_SAMPLES, len(clusters)),
                    )
                    cluster_counts = np.bincount(
                        cluster_ids,
                        minlength=len(clusters),
                    ).astype(float)
                    sampled_counts = cluster_counts[sampled_index].sum(axis=1)
                    for metric in metric_columns:
                        delta = (
                            frame[f"{metric}_candidate"]
                            - frame[f"{metric}_baseline"]
                        ).to_numpy(dtype=float)
                        cluster_sums = np.bincount(
                            cluster_ids,
                            weights=delta,
                            minlength=len(clusters),
                        )
                        draws = (
                            cluster_sums[sampled_index].sum(axis=1)
                            / sampled_counts
                        )
                        output.append(
                            {
                                "scope": scope,
                                "candidate_method": method,
                                "baseline_method": BASELINE_METHOD,
                                "period": period,
                                "metric": metric,
                                "cluster_type": cluster_type,
                                "n": len(frame),
                                "clusters": len(clusters),
                                "candidate_minus_baseline": float(
                                    delta.mean()
                                ),
                                "bootstrap_p025": float(
                                    np.quantile(draws, 0.025)
                                ),
                                "bootstrap_p975": float(
                                    np.quantile(draws, 0.975)
                                ),
                                "probability_candidate_better": float(
                                    np.mean(draws < 0)
                                ),
                            }
                        )
    return pd.DataFrame(output)


def pool_profile_audit(
    templates: pd.DataFrame,
    targets: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    grouped_donors = {
        (season, pos): group.reset_index(drop=True)
        for (season, pos), group in templates.groupby(["season", "pos"])
    }
    donor_seasons = sorted(templates.season.unique())
    donors_by_origin_pos = {}
    for season in sorted(targets.season.unique()):
        for position in builder.POSITIONS:
            donors = pd.concat(
                [
                    grouped_donors[(donor_season, position)]
                    for donor_season in donor_seasons
                    if donor_season < season
                    and (donor_season, position) in grouped_donors
                ],
                ignore_index=True,
            )
            donors_by_origin_pos[(season, position)] = donors[
                donors.template_eligible.eq(1)
            ].reset_index(drop=True)

    rows = []
    for target in targets.itertuples(index=False):
        eligible = donors_by_origin_pos[(target.season, target.pos)]
        pools = {
            method: pruning.selected_pool(target, eligible, METHODS[method])
            for method in [BASELINE_METHOD, PRIMARY_METHOD]
        }
        donor_keys = {}
        metrics = {}
        for method, pool in pools.items():
            donors = pool["donors"]
            probabilities = pool["probabilities"]
            donor_keys[method] = set(
                zip(
                    donors.player_key.astype(str),
                    donors.season.astype(int),
                )
            )
            metrics[method] = {
                "height_distance": float(
                    np.sum(
                        probabilities
                        * np.abs(
                            donors[HEIGHT_FEATURE].to_numpy(dtype=float)
                            - float(getattr(target, HEIGHT_FEATURE))
                        )
                    )
                ),
                "weight_distance": float(
                    np.sum(
                        probabilities
                        * np.abs(
                            donors[WEIGHT_FEATURE].to_numpy(dtype=float)
                            - float(getattr(target, WEIGHT_FEATURE))
                        )
                    )
                ),
                "effective_sample_size": float(
                    1 / np.square(probabilities).sum()
                ),
            }
        overlap = len(
            donor_keys[BASELINE_METHOD].intersection(
                donor_keys[PRIMARY_METHOD]
            )
        ) / len(donor_keys[BASELINE_METHOD])
        rows.append(
            {
                "player_key": target.player_key,
                "player": target.player,
                "pos": target.pos,
                "season": int(target.season),
                "pool_overlap_share": overlap,
                "height_distance_delta": (
                    metrics[PRIMARY_METHOD]["height_distance"]
                    - metrics[BASELINE_METHOD]["height_distance"]
                ),
                "weight_distance_delta": (
                    metrics[PRIMARY_METHOD]["weight_distance"]
                    - metrics[BASELINE_METHOD]["weight_distance"]
                ),
                "effective_sample_size_delta": (
                    metrics[PRIMARY_METHOD]["effective_sample_size"]
                    - metrics[BASELINE_METHOD]["effective_sample_size"]
                ),
            }
        )
    audit = pd.DataFrame(rows)
    summaries = []
    for scope in ["all", "qb", "rb", "wr", "te", "wr_te"]:
        scoped = scope_frame(audit, scope)
        summaries.append(
            {
                "scope": scope,
                "n": len(scoped),
                "mean_pool_overlap_share": float(
                    scoped.pool_overlap_share.mean()
                ),
                "median_pool_overlap_share": float(
                    scoped.pool_overlap_share.median()
                ),
                "mean_height_distance_delta": float(
                    scoped.height_distance_delta.mean()
                ),
                "mean_weight_distance_delta": float(
                    scoped.weight_distance_delta.mean()
                ),
                "mean_effective_sample_size_delta": float(
                    scoped.effective_sample_size_delta.mean()
                ),
            }
        )
    return audit, pd.DataFrame(summaries)


def write_summary(
    results_dir: Path,
    league: str,
    target_count: int,
    summaries: pd.DataFrame,
    bootstrap: pd.DataFrame,
    pool_summary: pd.DataFrame,
    coverage: pd.DataFrame,
    runtime_seconds: float,
) -> None:
    focus = summaries[
        summaries.method.isin([BASELINE_METHOD, PRIMARY_METHOD])
        & summaries.period.isin(["all_2017_2025", "temporal_2023_2025"])
    ][
        [
            "scope",
            "period",
            "method",
            "n",
            "ppg_crps",
            "contribution_crps",
            "played_crps",
            "plus3_brier",
            "impact_brier",
            "impact_auc",
        ]
    ]
    bootstrap_focus = bootstrap[
        bootstrap.candidate_method.eq(PRIMARY_METHOD)
        & bootstrap.period.isin(["all_2017_2025", "temporal_2023_2025"])
        & bootstrap.metric.isin(
            [
                "ppg_crps",
                "contribution_crps",
                "played_crps",
                "plus3_brier_row",
                "impact_brier_row",
            ]
        )
    ]
    text = f"""# Height/Weight Weekly-Template Replay ({league})

## Scope

- Strict rolling target seasons: 2017-2025.
- Held-out player-seasons: {target_count:,}.
- Primary comparison: `{PRIMARY_METHOD}` versus `{BASELINE_METHOD}`.
- Measurements come from the existing nflverse player master.
- Every donor precedes its target season.
- The production pool size, kernel, recency prior, donor cap, and joint outcome
  transport are unchanged.
- Production code and databases are unchanged.

## Coverage

{reference.markdown_table(coverage)}

## Outcome summary

{reference.markdown_table(focus)}

## Primary clustered comparisons

{reference.markdown_table(bootstrap_focus)}

Lower CRPS and Brier scores are better. `candidate_minus_baseline < 0` favors
the height/weight matcher.

## Pool-composition audit

{reference.markdown_table(pool_summary)}

Negative size-distance deltas mean the primary candidate selected donors closer
to the target on that measurement. Pool overlap is the share of baseline top-80
donors retained.

Runtime: {runtime_seconds:.1f} seconds.
"""
    (results_dir / "summary.md").write_text(text, encoding="utf-8")


def main() -> None:
    args = parse_args()
    league = str(args.league).lower()
    v2_database = (
        Path(args.v2_db).resolve()
        if args.v2_db is not None
        else Path(builder.resolve_v2_database(league=league)).resolve()
    )
    results_dir = args.results_dir.resolve()
    results_dir.mkdir(parents=True, exist_ok=True)
    started = time.perf_counter()

    builder.set_active_league(league)
    base.builder.LEAGUE = league
    pruning.METHODS = METHODS
    pruning.METHOD_METADATA = METHOD_METADATA
    pruning.BASELINE_METHOD = BASELINE_METHOD
    pruning.PERIODS = PERIODS

    max_season = builder.get_daily_max_template_season()
    measurables = load_measurables(v2_database)
    projections = builder.load_historical_projection_context(
        max_season,
        v2_database=v2_database,
    )
    weekly = builder.load_weekly_points(max_season, league=league)
    templates = builder.build_weekly_templates(
        projections,
        weekly,
        league=league,
    )
    templates = reference.reattach_template_player_keys(templates, projections)
    templates = attach_measurable_features(templates, measurables)
    forecasts = base.load_production_oos_forecasts(max_season)
    target_templates = base.build_production_oos_target_templates(
        templates,
        forecasts,
    )
    targets = base.build_targets(target_templates)

    predictions = pruning.run_replay(templates, targets)
    expected_rows = len(targets) * len(METHODS)
    if len(predictions) != expected_rows:
        raise AssertionError(
            f"Expected {expected_rows} predictions; found {len(predictions)}."
        )
    predictions = attach_target_profile(predictions, targets)
    predictions = reference.refresh_row_event_losses(predictions)

    coverage = coverage_audit(templates, targets)
    summaries = pd.concat(
        [
            grouped_period_summaries(predictions, scope)
            for scope in ["all", "qb", "rb", "wr", "te", "wr_te"]
        ],
        ignore_index=True,
    )
    bootstrap = clustered_bootstrap(predictions)
    pool_audit, pool_summary = pool_profile_audit(templates, targets)
    runtime_seconds = time.perf_counter() - started

    predictions.to_csv(results_dir / "target_predictions.csv", index=False)
    METHOD_METADATA.to_csv(results_dir / "method_metadata.csv", index=False)
    coverage.to_csv(results_dir / "feature_coverage.csv", index=False)
    summaries.to_csv(results_dir / "summary_by_scope_period.csv", index=False)
    bootstrap.to_csv(results_dir / "clustered_bootstrap.csv", index=False)
    pool_audit.to_csv(results_dir / "pool_profile_audit.csv", index=False)
    pool_summary.to_csv(results_dir / "pool_profile_summary.csv", index=False)
    metadata = {
        "league": league,
        "v2_database": str(v2_database),
        "nflverse_players_url": NFLVERSE_PLAYERS_URL,
        "max_template_season": int(max_season),
        "target_rows": int(len(targets)),
        "prediction_rows": int(len(predictions)),
        "method_count": int(len(METHODS)),
        "baseline_method": BASELINE_METHOD,
        "primary_method": PRIMARY_METHOD,
        "recency_half_life": RECENCY_HALF_LIFE,
        "bootstrap_samples": BOOTSTRAP_SAMPLES,
        "runtime_seconds": runtime_seconds,
        "production_changed": False,
    }
    (results_dir / "run_metadata.json").write_text(
        json.dumps(metadata, indent=2),
        encoding="utf-8",
    )
    write_summary(
        results_dir,
        league,
        len(targets),
        summaries,
        bootstrap,
        pool_summary,
        coverage,
        runtime_seconds,
    )
    print(
        summaries[
            summaries.scope.eq("all")
            & summaries.period.eq("all_2017_2025")
        ][
            [
                "method",
                "ppg_crps",
                "contribution_crps",
                "played_crps",
                "plus3_brier",
                "impact_brier",
                "impact_auc",
            ]
        ]
        .round(6)
        .to_string(index=False),
        flush=True,
    )


if __name__ == "__main__":
    main()
