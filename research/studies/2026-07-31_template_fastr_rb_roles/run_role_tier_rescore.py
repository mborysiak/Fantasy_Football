"""Apply the frozen role-tier policy to the nflfastR RB role replay."""
...
phase_a = load_module("rb_role_phase_a", ROLE_DIR / "run_phase_a_rescore.py")
phase_b = load_module("rb_role_phase_b", ROLE_DIR / "run_phase_b_rescore.py")
...
def rb_position_slices(predictions: pd.DataFrame) -> pd.DataFrame:
    masks = phase_a.tier_masks(predictions)
    scopes = {
        "rb_core_strict": predictions.pos.eq("RB") & masks["core_strict"],
        "rb_core_main": predictions.pos.eq("RB") & masks["core_main"],
        "rb_core_broad": predictions.pos.eq("RB") & masks["core_broad"],
        "rb_depth_main": predictions.pos.eq("RB") & ~masks["core_main"],
        "rb_all_saved": predictions.pos.eq("RB"),
    }
    rows = []
            ...
            for period, (start, end) in phase_a.PERIODS.items():
                ...
                for method, group in period_frame.groupby("method"):
                    ...
                    for metric in phase_a.METRICS:
                        ...
                        row[f"{metric}_delta"] = candidate - baseline_value
                        row[f"{metric}_relative_delta"] = (
                            candidate / baseline_value - 1
                        )
                    rows.append(row)
...
def main() -> None:
    ...
    rb_slices = rb_position_slices(predictions)
    ...
    rb_slices.to_csv(RESULTS / "rb_position_slices.csv", index=False)
    ...
    slice_focus = rb_slices[
        rb_slices.scope.isin(["rb_core_main", "rb_depth_main"])
        & rb_slices.period.isin(
