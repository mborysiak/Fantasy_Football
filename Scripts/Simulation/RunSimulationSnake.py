...
my_picks = get_picks(initial_pick)

pick_prob_df = pd.read_sql_query('''SELECT * FROM PickProb''', conn_sim)
player_adp = pick_prob_df[['player', 'adp']].drop_duplicates().set_index('player')
pick_prob_df = pick_prob_df[pick_prob_df.pick.isin(my_picks)].groupby('player').agg({'pick_prob': 'sum'}).reset_index()
...
salaries = pd.read_sql_query(f'''SELECT player, salary
                                    FROM Salaries
                                    WHERE year={set_year}
                                        AND league='{league}' ''', conn_sim).reset_index(drop=True)
...
salaries.index = salaries.index + 1
league_info['salary_cap'] = salaries.loc[salaries['index'].isin(my_picks), 'salary'].sum() * 1.25
...
sim = FootballSimulation(pts_dict, conn_sim, table_vers, set_year, league, iterations, pick_prob_df)
...
d = pd.merge(d, player_adp, left_index=True, right_index=True)
player_list = []
for pl, row in d.sort_values(by='adp')[['Salary', 'Position', 'adp']].iterrows():
    if row.Position != 'FLEX':
        player_list.append([row.Position, pl, row.adp,  0])
d = d.drop('adp', axis=1)

pick_df = pd.DataFrame(player_list, columns=['Position', 'Player', 'ADP', 'Salary'])
pick_df['My Team'] = 'No'
...
def update_output(n_clicks, drafted_data, drafted_columns):
    ...
    avg_sal['Points Added'] = avg_sal['Points Added'].astype('int')
    avg_sal['to_sort'] = (avg_sal.PercentDrafted + avg_sal['Points Added']) / 2
    avg_sal = avg_sal.sort_values(by='to_sort').drop('to_sort', axis=1)
