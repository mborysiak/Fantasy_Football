...
def pts_over_expectation(df, lr_mean):
    
    # initialize list for storing values
    pts_over_exp = []

    # loop through each of the top 20 chosen players
    for _, row in df.iterrows():

        # find current players position and points mean
        cur_pl = d[(d.index==row.Player) & (d.Position!='FLEX')].reset_index()
        cur_pos = cur_pl.Position
        cur_pts = cur_pl.iloc[:,2:999].mean(axis=1)

        # convert the player information to X_test format to predict
        row = pd.DataFrame(row).T
        row['Position'] = cur_pos
        row = row[['Position', 'AverageSalary']]
        row.columns = ['Position', 'Salary']
        X_test = create_X_test(row, test_template)

        # determine extra pts above what was paid
        extra_pts = cur_pts - lr_mean.predict(X_test)
        pts_over_exp.append(extra_pts[0])
...
remain_init, _ = salary_proportions(my_team_df, proport, league_info['salary_cap'])
X_init = create_X_test(remain_init, test_template)
team_dist = create_pt_dist(X_init, pd.DataFrame(), lr_mean, lr_std)
...
#     INPUT
#     -----
#     Dataframe containing players + salaries to be dropped from selection

#     OUTPUT
#     ------
#     Dictionary containing dropped player + salaries for passing into simulation
#     '''
#     to_drop = {}
#     to_drop['players'] = []
#     to_drop['salaries'] = []
...
#     INPUT
#     -----
#     Dataframe containing players + salaries to be added to my team

#     OUTPUT
#     ------
#     Dictionary containing my team player + salaries for passing into simulation
#     '''
#     to_add = {}
#     to_add['players'] = []
#     to_add['salaries'] = []
...
#     extra_pts = pts_over_expectation(avg_sal, lr_mean)
#     avg_sal['ExtraPoints'] = extra_pts
...
#     pick_bar = create_bar( list(avg_sal.PercentDrafted),list(avg_sal.Player), text=list(avg_sal.PercentDrafted))
#     sal_bar = create_bar(list(avg_sal.AverageSalary), list(avg_sal.Player), 
#                          color_str='rgba(250, 190, 88, 1)', text=avg_sal.AverageSalary)
#     pts_bar = create_bar( list(avg_sal.ExtraPoints),list(avg_sal.Player), text=list(avg_sal.ExtraPoints))
#     gr_fig = create_fig_layout(sal_bar, pick_bar, pts_bar)
...
#     hist_fig, pt_perc = create_pt_dist(X_test, selected_df, lr_mean, lr_std)

#     # update team information table
#     team_info_update = pd.DataFrame({'Mean Points': [pt_perc['50']], 
