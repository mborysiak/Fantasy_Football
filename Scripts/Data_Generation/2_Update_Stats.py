...
for pos in ['QB', 'RB', 'WR', 'TE']:
    ...
    df = dm_daily.read(f'''SELECT * 
                           FROM {pos}_Stats
                           WHERE (season >= 2021 AND week >=15 AND week <= 17)
                                  OR (season < 2021 AND week>=14 AND week <= 16)
                        ''', 'FastR_Beta')
    ...
    df_all = df_all.sort_values(by=['year', 'fantasy_pts_per_game'], ascending=[False, False]).reset_index(drop=True)
    df_all.player = df_all.player.apply(dc.name_clean)
    dm_ff.write_to_db(df_all, 'Season_Stats_Playoffs', f'{pos}_Stats', if_exist='replace')
# %%
