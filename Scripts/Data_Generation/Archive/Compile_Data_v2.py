...
df = dm_daily.read('''SELECT * FROM RB_Stats''', 'FastR_Beta')
...
for agg_type in ['sum', 'mean', perc_agg80, 'max', 'std']:
    print(i, agg_type)
    agg_stats = {}
    for c in stat_cols:
...
df_all['year_shift'] = df_all.groupby('player')['season'].shift(-1)
df_all = df_all[df_all.year_shift == df_all.season+1].reset_index(drop=True)
...
Xy = df_all.drop([c for c in df_all if 'fantasy_pts' in c], axis=1).dropna().reset_index(drop=True)
Xy = Xy.rename(columns={'season': 'year'})
Xy['team'] = 'team'
Xy['week'] = 1
...
df_all[(df_all.player=='Giovani Bernard')]
