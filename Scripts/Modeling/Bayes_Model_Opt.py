...
# set core path
path = 'c:/Users/mborysia/Documents/Github/Fantasy_Football/'
...
def calc_f1_score(**args):
    ...
    skf = StratifiedKFold(n_splits=5, random_state=i*3+i*17+14, shuffle=True)
...
for model in list(class_models.keys()):
    ...
    best_params_model = res_gp.x_iters[np.argmin(combined_scores[cnter-100:cnter])]
...
# results_no_hold = val_results(models_list, val_scores, combined_scores, test_scores)
results_no_hold

results_opt_score_10fold = val_results(models_list, val_scores,
...
def calc_rmse(**args):
    ...
    # calculate the RMSE and MAE of the ensemble predictions
    comb_score = round(np.sqrt(mean_squared_error((val_predictions, y_vals, sample_weight=wts)), 3)
    ...
    if opt_score < globals()['best_opt']:
        ...
        if i > 25:
            ...
            print('ValScore:',val_score)
            print('TestScore:', test_score)
            print('CombinedScore:', comb_score)
        
    if m_score < globals()['best_combined']:
        globals()['best_combined'] = comb_score
    ...
    globals()['val_scores'].append(val_score)    
    globals()['test_scores'].append(test_score)
    globals()['combined_scores'].append(comb_score)
